...
from data.warp import cover11,cover1,cover0
import numpy as np
...
def run(weights=ROOT / 'yolov3.pt',  # model.pt path(s)
    ...
    model = DetectMultiBackend(weights, device=device, dnn=dnn).eval()  # .eval()
    ...
    # %%#
    patch_name='patch_990'
    #attack_img0 = cv2.imread('./data/patch-all/3/'+patch_name+'.png')  # patch
    attack_img0 = cv2.imread('./data/patch/patch_96.png')
    ...
    for path, _, im0s, vid_cap, s in dataset:
        ...
        scene_index=int(path.split("_")[2][-1])
        frame_index=int(path.split("_")[4].split(".")[0])

        # %背景图加patch
        pts = np.load('./data/pts/data0' + str(scene_index) + '.npy')  # 场景的所有坐标8*231
        ori_pts = np.around(pts[:,frame_index-1].reshape(4, 2)).astype(np.float32)  # 背景图对应的坐标

        test_pts = np.array([[0, 0], [2790 - 1, 0], [2790 - 1, 1260 - 1], [0, 1260 - 1]], dtype=np.float32)  # patch对应的坐标
        im = cover1(im0s, attack_img0, ori_pts, test_pts, device)  # (1080, 1920, 3)
        im_0=im.copy()

        # 加patch后的图像预处理
        im=letterbox(im,imgsz, stride=32, auto=True)[0]     #(384, 640, 3)
        im = im.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB   #(3, 384, 640)
        im = np.ascontiguousarray(im)    #(3, 384, 640)

        im = torch.from_numpy(im).to(device)    #torch.Size([3, 384, 640])
        ...
        # Process predictions
        for i, det in enumerate(pred):  # 每张图片  len(pred)=1
            #seen += 1
            if webcam:  # batch_size >= 1
                p, im0, frame = path[i], im0s[i].copy(), dataset.count
                s += f'{i}: '
            else:
                p, im0, frame = path, im_0.copy(), getattr(dataset, 'frame', 0)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        annotator.box_label(xyxy, label, color=colors(c, True))    #图像结果处理
                        if save_crop:
            ...
            if flag0 == 1:
                flag += 1
                print("flag:", flag)
            cv2.imwrite(save_path.split(".")[0]+".png", im2)
    ...
    # 统计攻击成功率
    print(patch_name+": Success={}/{},   Ratio={} ".format(flag,count,flag/count))
