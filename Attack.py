...
def run(weights=ROOT / 'yolov3.pt',  # model.pt path(s)
    ...
    #导入mask
    mask=cv2.imread('./data/mask/mask_dog.png')
    # print(type(mask))
    # print(type(np.array(mask)))
    mask=Variable(torch.from_numpy(np.array(mask)).to(device).float())

    #定义优化的patch
    modifier=Variable(torch.zeros(1260,2790,3).to(device).float())
    ori_patch = cv2.imread('./data/patch/bird.jpg')
    ...
    for epoch in range(1,max_epoches+1):
        ...
        for path, _, im0s, __, ___ in dataset:
            ...
            patch = modifier + ori_patch
            im11=cover0_mask_robust(im1s, patch, mask, ori_pts, test_pts, device)   # (1080, 1920, 3)
            ...
            # pred_two = model(im_two, augment=augment, visualize=visualize)     #(1,num_box,85), 85为box(4)+box_conf(1)+cls_conf(80)
            # pred1_two = non_max_suppression1(pred_two, conf_thres, iou_thres, classes, agnostic_nms, max_det=max_det)
            # pred_two =pred_two[pred_two[...,4]>thresh]   #取检测框概率>thresh的结果
        ...
        if (epoch_num%20)==0:
            cv2.imwrite("./data/result/patch_"+str(epoch_num)+".png", np.uint8(torch.clamp(modifier+ori_patch,0,255).cpu().detach()))
